...
int Gmp_to_char(mpz_t large_number, char chaine[SIZE_BUFFER] ) {
    ...
    if (result < 0) {
        Write_log(LOG_FILE, "Gmp_to_char:25:GMP_sprintf ERROR");
        return 25; // Code d'erreur pour indiquer que gmp_sprintf a échoué
    }
...
int Char_to_gmp(char chaine[SIZE_BUFFER], mpz_t large_number) {
    ...
    if (mpz_set_str(large_number, chaine, 10) != 0) {
        Write_log(LOG_FILE, "Char_to_gmp:26:CONVERSION ERROR");
        return 26; // Code d'erreur pour indiquer que la conversion n'a pas marchée
    }
    /*    if (DEBUG == 1) { // Dev Debug
